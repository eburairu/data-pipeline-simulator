...
### 4. 可視化とモニタリング
*   **Pipeline Flow (シミュレーション可視化)**: データパイプライン全体のフローをDAG（有向非巡回グラフ）として可視化します。
    *   **依存関係の可視化**: Mapping Task間の依存関係や、Task Flowの構成要素、およびデータソース（ファイル、DBテーブル）との接続関係を自動的に描画します。
    *   **リアルタイムステータス**: 実行中のタスクやデータ転送の状況をアニメーションで表示します。
