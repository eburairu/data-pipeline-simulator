...
import ReactFlow, { Background, Controls, Panel, type Node, type Edge, MarkerType, type ReactFlowInstance, useNodesState, useEdgesState, type NodeChange, type EdgeChange } from 'reactflow';
...
import TransformationToolbar from './mapping/TransformationToolbar';
...
const MappingDesigner: React.FC<MappingDesignerProps> = ({ executionStats, readOnly = false, initialMappingId }) => {
        ...
        return (
            <div className="h-[600px] flex flex-col border rounded bg-white shadow-sm">
                ...
                <div className="flex-grow flex flex-col md:flex-row overflow-hidden relative">
                    <TransformationToolbar readOnly={readOnly} addTransformation={addTransformation} />
                    ...
                    <div className="flex-grow relative bg-gray-50">
                        <ReactFlow
                            ...
                            onEdgeClick={(_, edge) => !readOnly && setSelectedEdgeId(prev => prev === edge.id ? null : edge.id)}
                            onConnect={(params) => {
                                if (readOnly || !editingMapping || !params.source || !params.target) return;
                                if (editingMapping.links.some(l => l.sourceId === params.source && l.targetId === params.target)) return;
                                if (params.source === params.target) return;
                                const newLink = { id: `l_${crypto.randomUUID().substring(0, 8)}`, sourceId: params.source, targetId: params.target };
                                setEditingMapping(prev => prev ? ({ ...prev, links: [...prev.links, newLink] }) : null);
                            }}
                            ...
                            <Panel position="top-right">
                                <div className="flex gap-2">
                                    {selectedEdgeId && !readOnly && (
                                        <button onClick={deleteSelectedEdge} className="flex items-center gap-2 bg-red-500 text-white px-3 py-2 rounded shadow border border-red-600 hover:bg-red-600 text-sm font-medium cursor-pointer" title="Delete selected connection">
                                            <Trash2 size={16} /> Delete
                                        </button>
                                    )}
                                    <button onClick={onLayout} className="flex items-center gap-2 bg-white px-3 py-2 rounded shadow border border-gray-200 hover:bg-gray-50 text-sm font-medium text-gray-700 cursor-pointer" title="Auto-align nodes">
    ...
    return (
        <MappingList
            ...
            handleDelete={(id) => { if (!readOnly) removeMapping(id); }}
