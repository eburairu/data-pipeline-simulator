# 機能仕様書: Data Pipeline Simulator ベースライン
...
### ユーザーストーリー 1 - データパイプラインのシミュレーション実行 (優先度: P1)

ユーザーはデータ生成からデータベースへの格納までのデータパイプライン全体をシミュレートし、各ステージでのファイルの動きを確認できる。

**この優先度の理由**: 本アプリケーションの核となる価値であり、データフローの可視化と理解を目的としているため。

**独立テスト**: 設定されたジョブに従ってファイルが生成・移動・処理され、最終的にDBにレコードが格納されることを確認する。

**受け入れシナリオ**:

1. **前提条件** デフォルトの設定状態, **操作** "Start Auto-Run" ボタンをクリック, **期待結果** ファイルが定期的に生成され、パイプラインを通って移動し、DBに格納される様子が可視化される。
2. **前提条件** シミュレーション実行中, **操作** "Stop Auto-Run" ボタンをクリック, **期待結果** 新規のファイル生成と移動処理が停止する。
3. **前提条件** 停止状態, **操作** "Create Source File" ボタンをクリック, **期待結果** 単発でソースファイルが生成され、パイプライン処理が実行される。

### ユーザーストーリー 2 - パイプライン構成の可視化 (優先度: P1)

ユーザーは現在の設定（ホスト、パス、ジョブ）に基づいたパイプラインの構成図を動的に確認できる。

**この優先度の理由**: 複雑な設定を直感的に理解するために不可欠であるため。

**独立テスト**: 設定変更（新しいホストやジョブの追加）が即座にダイアグラムに反映されることを確認する。

**受け入れシナリオ**:

1. **前提条件** 設定画面, **操作** 新しいCollectionジョブを追加（異なるターゲットホストを指定）, **期待結果** ダイアグラム上に新しいノードとエッジが表示される。
2. **前提条件** シミュレーション実行中, **操作** ファイル転送が発生, **期待結果** 対応するエッジやノードがアニメーションまたはハイライト表示される。

### ユーザーストーリー 3 - 詳細なインフラとジョブ設定 (優先度: P2)

ユーザーはシミュレーションの前提となるインフラ（ホスト、ディレクトリ）や、各ステージの挙動（帯域幅、遅延、フィルタ）を詳細に設定できる。

**この優先度の理由**: 様々なシナリオ（低帯域幅、高レイテンシ、特定のファイル名パターンなど）をテストするために必要。

**独立テスト**: 設定を変更し、それがシミュレーションの挙動（処理時間やファイル移動先）に反映されることを確認する。

**受け入れシナリオ**:

1. **前提条件** Collection設定, **操作** 帯域幅を低く設定, **期待結果** ファイル転送にかかる時間が増加する（アニメーションの滞在時間が長くなる）。
2. **前提条件** インフラ設定, **操作** 新しいホストを追加, **期待結果** そのホストがジョブ設定のプルダウンメニューで選択可能になる。
