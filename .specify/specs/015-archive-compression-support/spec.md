...
- `JobMonitor.tsx`: `'archive'` 型のログ表示、アイコン、フィルタを追加。
- `PipelineFlow.tsx`: アーカイブジョブノードの描画と実行時のアニメーション表示を実装。
...
- **完全な機能統合**: 新しいジョブタイプを追加する際は、「設定」「制御」「監視」の三要素すべてに統合されていることを確認する（再発防止策）。
